...
__global__ void internalReOrder(unsigned long long int *parents, int ulonglongRequired, unsigned int *parentVals, int max)
{
    ...
    if (max > id)
    {
        ...
        unsigned long long int tmpLowest[ulonglongRequired];
        unsigned long long int tmpHighest[ulonglongRequired];
        //TODO
        ...
        for(int i =0; i < ulonglongRequired ; i++){
            parents[highestIndex*ulonglongRequired+i] = parents[(bId + threadsInBlockIsland-1)*ulonglongRequired+i];
        }
        for(int i =0; i < ulonglongRequired ; i++){
            parents[(bId + threadsInBlockIsland-1)*ulonglongRequired+i]=tmpHighest[i];
        }
    }
}

__global__ void migration(unsigned long long int *parents, int ulonglongRequired , int max)
{
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (max > id)
    {
        // Migration - the last chromosome replaces the first chromosome of the next block
        int index = ((id + 1) * threadsInBlockIsland - 1)*ulonglongRequired;
        if (index >= max)  index = index - max;
        int replaceIndex = ((id + 1) * threadsInBlockIsland)*ulonglongRequired;
