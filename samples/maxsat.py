 ...
 __global__ void evaluation(unsigned long long int *parents, int ulonglongRequired, int *chromosomesResults, int* constantArray, int constantArraySize, int max)
{
    ...
    if(max>id){
        int tmpVar = 0;
        for(int i = 0 ;i<constantArraySize;i+=satSize){
            tmpVar+=1;
            for(int ii = 0 ;ii<satSize;ii++){
                int constantArrayIndex = i+ii;
                int constantArrayAbsumber = constantArray[constantArrayIndex]; 
                if(constantArray[constantArrayIndex]<0){
                    constantArrayAbsumber=constantArrayAbsumber*-1;
                }
                int tmpParentIndex = constantArrayAbsumber/64+id;
                int tmpParentDigit = constantArrayAbsumber% 64;
                if(constantArray[constantArrayIndex]<0){
                    //negative number but receives 1
                    if((parents[tmpParentIndex] >> tmpParentDigit) & 1){
                        tmpVar-=1;
                        break;
                    }
                }else{
                    //positive number but receives 0
                    if(!((parents[tmpParentIndex] >> tmpParentDigit) & 1)){
                        tmpVar-=1;
                        break;
                    }
...
p1 = PyCUGA( mutationThreshold = 0.1, isTime = False, time = 0, constArr = problemSet, chromosomeSize = 1024, stringPlaceholder=stringPlaceholder)
p1. launchKernel(islandSize = 32, blockSize = 128, chromosomeNo = 1024, migrationRounds = 5,rounds = 1000)